@extends('master')

@section('title', 'Cafde\'s Quest for Satisfaction')

@section('content')
    <div class="row">
        <div id="cafde-banner" class="col-xs-12">
            <h2>Cafde's Quest for Satisfaction</h2>
        </div>
    </div>
    <div class="row">
        <div class="col-lg-6 game-info">
            <img src="/images/games/cafde/screen3.png" class="pull-left" />
            <a href="/games/cafde/download" class="btn btn-lg btn-primary-ghost pull-right">Download</a>
            <h3>Cafde's Quest for Satisfaction</h3>
            <p>
                Dunky dunky, what's in the trunky? Not satisfaction, that's for sure! <strong>Cafde's Quest</strong> answers the burning question: <em>When life gives you sugar cubes, what do you do?</em> The answer? That's up to player interpretation. Regardless, help Cafde grab his trusty <span id="special-text">ANIME PRO</span> mug and keep his coffee untainted for as long as possible. That's right, we put your brand in their hand! There is no winning in this cruel world.
            </p>
            <p>
                Move around with the arrow keys and levitate with the space bar.
            </p>
            <p>
                This game was made in a single day.
            </p>
            <div class="requirements">
                <h4>Requirements:</h4>
                <span class="require">OS:</span> Windows<br>
                <span class="require">Dependencies:</span> Java<br>
                <span class="require">Space:</span> 10MB
            </div>
        </div>
        <div class="col-lg-6">
            <div class="screen-holder">
                <img src="/images/games/cafde/screen2.png" />
            </div>
        </div>
    </div>
@endsection
